...
class SessionController {
  async login(req: RequestWithType<Partial<Usuario>>, res: Response): Promise<Response> {
    const { email, senha } = req.body;

    const usuario = await getRepository(Usuario).findOne({ email });
  ...
  async registration(req: RequestWithType<Partial<Usuario>>, res: Response): Promise<Response> {
    const data = req.body;

    try {
      const results = await getRepository(Usuario).save(data);
